...
The packages plyr and ggplot2 will be used for preoprocessing and data analysis.  Once the data has been imported, dates were converted into Date format.  An additional factor is created that specifies whether a day is a weekend or weekday. 
...
For this section, NA values are removed for analysis.  The mean number of steps is specfied in the summary below.
...
sums.plot <-ggplot(sums.df, aes(x=date, y=steps)) + geom_line()
sums.plot
...
print("Summary activity for interpolated values")
summary(activity.sums.ext)

print("Original values are below for comparison")
summary(activity.sums)

sums.df.ext <-ldply(activity.sums.ext)
names(sums.df.ext) <-c("date", "steps")
sums.df.ext$date <- as.Date(sums.df.ext$date)
sums.plot.int <-ggplot(sums.df.ext, aes(x=date, y=steps)) + geom_line()
sums.plot.int
